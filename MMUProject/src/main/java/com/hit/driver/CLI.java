...
/*try {
//process will start just when user will insert START command
while(!command.equals(START)){
if(command.equals(STOP))
{
	cout.write("Thank you!\n");
	cout.flush();
	return;
}
cout.write("Not a valid command!\n");
cout.flush();
command = cin.next();
}
//START command was insert, getting next command 
while(!command.equals(STOP)){
try {
	write(command);
} catch (JsonIOException | JsonSyntaxException | FileNotFoundException | InterruptedException | ExecutionException e) {
	e.printStackTrace();
}
//Just START or STOP commands will let user to continue 
command = cin.next();
while(!command.equals(START)){
	if(command.equals(STOP))
		break;
...
/*try {
			...
			default: //System.out.println("Not valid command");
				cout.write("This is not valid command!\n");
				cout.flush();
				break;
			}
		} catch (IOException e) {
